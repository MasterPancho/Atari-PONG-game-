...
#def song_one():
#    song_one = random.choice(["Mario Galaxy.mp3", "Title.mp3", "Megaman 2.mp3", "Hangar.mp3", "Gaur Plains.mp3", "Final Battle.mp3"])
...
#Creates a text given the type, font, and color
def text_objects(text, font, color=white):
    textSurface = font.render(text, True, color)
    return textSurface, textSurface.get_rect()

#Creates a button with its respective action
...
def game_intro():
    ...
    while intro:
        ...
        TextSurf, TextRect = text_objects(" Atari Pong Game", largeText)
...
def game_loop():
    ...
    while not gameExit:
        ...
        #Pressing "M" Key, wil lreturn you to the main menu
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_m:
                   game_intro()

        #PLAYER 1
        #KEYUP values are set to zero, and only values for KEYDOWN are given
        #since we wanted to be able to FULLY control the rackets' movement,
        #without any gravity involved. If the KEYUP were to have values,
        #the racket would move up or down automatically.

            #keydown event (Key is pressed)
            if event.type == pygame.KEYDOWN:
